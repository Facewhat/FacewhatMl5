...
XoW.File = function (toJid) {
  ...
  this.classInfo = 'File_' + _this.name;
  // endregion Fields
  ...
  this.getTypeDesc = function () {
    ...
    var desc = _this.name.substring(_this.name.lastIndexOf('.') + 1, _this.name.length) || '未知';
    desc += '文件';
    return desc;
  };
  ...
  this.getTrimmedName = function () {
    if (_this.name.length > 20) {
      return _this.name.substring(0, 20) + '...';
    }
    return _this.name;
  };

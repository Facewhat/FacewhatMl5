...
triggerHandler : function(proName, params) {
	XoW.logger.ms(this.classInfo + "triggerHandler()");
	var count = 0;
	for (var i = 0; i < this.handlers.length; i++) {
        var _handler = this.handlers[i];
        if(!_handler) {
	...
	XoW.logger.d(this.classInfo + " 本次触发" + count + "个回调");
	XoW.logger.me(this.classInfo + "triggerHandler()");
},	
};
return XoW;
}));
...
//	exit      : 'exit.gif'
  ...
  this.toString = function()
  {
    ...
    if(rootCN.length>0)
    {
      ...
      // 等待10毫秒，触发事件  expand和 focusClientNode 和 focusClientNode
      setTimeout(this.name +".expand('"+ rootCN[0].id +"', true); "+ 
    	        this.name +".focusClientNode('"+ rootCN[0].id +"'); "+ this.name +".atRootIsEmpty()",100);
//      setTimeout(function() {
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  var obj = XoW.utils.parseQueryString(url);
  var mi = '';
  var jid =''; 
  if(obj.type == 'groupprivate') {
	  // 只有群组中的人才显示状态
	  mi = "<mi class='layim_groupuser_state layim_groupuser_offline'>&nbsp;&nbsp;&nbsp;&nbsp;</mi>";
	  jid = obj.jid;
  } else {
	  jid = obj.groupjid;
  }
  
  var HTML = "<DIV noWrap='True'><NOBR data-id='" + jid + "' type='" + obj.type + "'>";
  if(!isRoot)
  {
  ...
  HTML += "<IMG "+
    ...
    "src='"+ this.icons[node.icon].src +"'>" + mi + "<A "+

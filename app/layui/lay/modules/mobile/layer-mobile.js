...
layui.define(function(exports){
  ...
  // add by cy for 避免点透事件 [20190331]
  //>layim-event触发后执行layer.open()后会窗口自动关闭，因自动触发其shade上的click
  //>在移动端，手指点击一个元素，会经过：touchstart --> touchmove -> touchend --> click
  //>已与贤心交流过了
  var isTouch = /Android|iPhone|SymbianOS|Windows Phone|iPad|iPod/.test(navigator.userAgent);

  //点触事件
  // mod by cy for 避免点透事件 [20190331]
  ...
  Layer.prototype.view = function(){
    ...
    var elem = that.elem = S('#'+that.id)[0];
	  // 在layer的回调中打开layer会导致id相同，故对调一下两行代码 mod by cy [20190416]
	  that.index = index++;
  ...
  var layer = {
    ...
    open: function(options){
      ...
      return o.index; // add by cy [20190408]
    },
    ...
    closeAll: function(){
	  ...
	  // 获取layer add by cy []20190416]
	  getLayer: function(index){
		  return S('#'+classs[0]+index)[0];
	  }
  };

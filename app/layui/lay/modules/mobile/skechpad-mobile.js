/**
 * add by zjy for 移动端画板 end【20190812】
 */
"use strict";
layui.use(['zepto']).define(['layer', 'form'], function (exports) {
    let $ = layui.zepto,
        _layer = layui.layer,
        layer = layui['layer-mobile'];
    let _classInfo = 'Sketchpad-mobile';
    let THIS = 'layim-this';
    ...
    let eleSketchpad = [
        '<div id ="drawer" style="width: 98%;height: 60%">'
        , '<div id = "canvasBegin">'
        , '<canvas  id="sketch">'
        , '</canvas>'
        , '</div>'
    ...
    function beginDrawing() {
        ...
        $('#drawer').ready(function () {
            ...
            document.getElementById("sketch").ontouchend = function (ev) {
                cast.open_down = false;
                if (cast.isRectangle === true) {
                    let offset = $('#sketch').offset();
                    let pos = {
                        x: ev.changedTouches[0].clientX - offset.left,
                        y: ev.changedTouches[0].clientY - offset.top
                    };
            ...
            document.getElementById("sketch").ontouchstart = function (ev) {
                cast.open_down = true;
                // cast.flag=true;
                if (cast.isRectangle === true) {
                    let offset = $('#sketch').offset();   //offset() 方法设置或返回被选元素相对于文档的偏移坐标。
                    cast.baseXY = {
                        x: ev.touches[0].clientX - offset.left,
                        y: ev.touches[0].clientY - offset.top
                    };
                }
                if (cast.isRound === true) {
                    let offset = $('#sketch').offset();
            ...
            document.getElementById("sketch").ontouchmove = function (ev) {
    ...
    function sendLoadImage(type) {
        ...
        return pSendInfo;
        // _client.sendFile(filename, fileLength, 'image/png', toFullJid, imagedata);
        XoW.logger.me(_classInfo, 'sendLoadImage()');
    }
    function getThisChat(){
        XoW.logger.ms(_classInfo, 'getThisChat()');
        // layimChat
        let $layimChat = getChatEle();
        if (!$layimChat || $layimChat.length === 0) {
            return null;
        }
        let index = $('.layim-chat-list .' + THIS).index();
        let cont = $layimChat.find('.layim-chat').eq(index);
        let to = JSON.parse(decodeURIComponent(cont.find('.layim-chat-tool').data('json')));
        return {
            elem: cont
            , data: to
            , textarea: cont.find('textarea')
        };
        XoW.logger.me(_classInfo, 'getThisChat()');
    };
    function getChatEle() {
        XoW.logger.ms(_classInfo, 'getChatEle()');
        let $layimChat=$('.layim-panel.layui-m-anim-left');
        return $layimChat; // jquery对象或zepto对象
        XoW.logger.me(_classInfo, 'getChatEle()');
    };
    ...
    exports('skechpad-mobile', new Sketchpad())
